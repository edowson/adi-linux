		...
		struct jesd204_link *lnk)
{
	...
	num = ADI_TO_PROFILE_NUM(axiadc_read(st, ADI_REG_TPL_STATUS));

	for (i = 0; i < num; i++) {
		last = (i == (num - 1));
		failed = false;

		axiadc_write(st, ADI_REG_TPL_CNTRL, ADI_PROFILE_SEL(i));
		d1 = axiadc_read(st, ADI_REG_TPL_DESCRIPTOR_1);
		d2 = axiadc_read(st, ADI_REG_TPL_DESCRIPTOR_2);
		...
		if (ADI_TO_JESD_M(d1) != lnk->num_converters * multi_device_link) {
			if (last)
				dev_warn(dev,
					...
					i, lnk->link_id, ADI_TO_JESD_M(d1),
					lnk->num_converters, multi_device_link);
			failed = true;
		}

		if (lnk->samples_per_conv_frame && ADI_TO_JESD_S(d1) !=
			lnk->samples_per_conv_frame) {
			if (last)
				dev_warn(dev,
					"profile%u:link_num%u param S mismatch %u!=%u\n",
					i, lnk->link_id, ADI_TO_JESD_S(d1),
					lnk->samples_per_conv_frame);
			failed = true;
		}
		...
		if (ADI_TO_JESD_F(d1) != lnk->octets_per_frame) {
			if (last)
				dev_warn(dev,
					...
					i, lnk->link_id, ADI_TO_JESD_F(d1),
		...
		if (ADI_TO_JESD_NP(d2) != lnk->bits_per_sample) {
			if (last)
				dev_warn(dev,
					...
					i, lnk->link_id, ADI_TO_JESD_NP(d2),
					lnk->bits_per_sample);
			failed = true;
		}
