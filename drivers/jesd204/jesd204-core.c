...
static int jesd204_dev_init_deferred_states(struct jesd204_dev *jdev,
					    struct device_node *np)
{
	unsigned int deferred_states[JESD204_FSM_STATES_NUM];
	int fsm_state;
	int i, ret;

	ret = of_property_read_variable_u32_array(np, "jesd204-deferred-states",
						  deferred_states,
	...
	for (i = 0; i < ret; i++) {
		if (deferred_states[i] >= JESD204_FSM_STATES_NUM) {
			pr_err("%pOF: Invalid state ID %u\n", np,
			       deferred_states[i]);
			return -EINVAL;
		}
		jdev->deferred_states[deferred_states[i]] = true;
		fsm_state = deferred_states[i] + JESD204_STATE_FSM_OFFSET;
		pr_info("%pOF: deferred state: '%s'\n", np,
...
static struct jesd204_dev *jesd204_dev_alloc(struct device_node *np)
{
	...
	ret = jesd204_dev_init_deferred_states(jdev, np);
	if (ret)
		goto err_free_id;
